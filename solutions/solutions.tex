...
\textbf{The linearization.} Starting from the same ODE
\(\dot{w} = 2w(s - w)\), with
...
\textbf{(a)} The linearized ODE \(\dot{w} = 2w_0(s - w)\) is first-order
...
\textbf{(b)} Restoring the mode index, each mode satisfies
