...
\textbf{(a)} When all \(W_l\) are diagonal, the product
...
\textbf{(b)} For \(L = 2\), the gradient equations are:
...
\textbf{(c)} At the critical point \(W = P_S M\) with
...
analysis (extending part (b)), the direction corresponding to
...
\textbf{(d)} Under the group action:
...
\(W\). The global minima form the fiber \(\mu^{-1}(M)\), which contains
...
\textbf{(b)} Let \(E := M - W_2 W_1\) denote the residual. Compute:
...
\textbf{(c.i)} Apply the product rule:
...
\textbf{(c.ii)} For diagonal matrices, \(W_1 = \text{diag}(a_\alpha)\)
and \(W_2 = \text{diag}(b_\alpha)\) with
\(W = \text{diag}(a_\alpha b_\alpha)\). Balancedness gives
\(b_\alpha^2 = a_\alpha^2\) for each \(\alpha\),
i.e.~\(|b_\alpha| = |a_\alpha|\) (with matching signs for positive
entries).

Now \(W_1^\top W_1 = \text{diag}(a_\alpha^2)\) and
\(W^\top W = \text{diag}(a_\alpha^2 b_\alpha^2)\). Using
\(b_\alpha^2 = a_\alpha^2\):
...
\textbf{(c.iii)} By the same argument,
\(W_2 W_2^\top = \text{diag}(b_\alpha^2) = \text{diag}(a_\alpha^2)\)
(using \(b_\alpha^2 = a_\alpha^2\)). And
\(WW^\top = \text{diag}(a_\alpha^2 b_\alpha^2) = \text{diag}(a_\alpha^4)\),
so \((WW^\top)^{1/2} = \text{diag}(a_\alpha^2) = W_2 W_2^\top\).
\(\square\)

\textbf{(c.iv)} Substituting into (c.i):

\[\dot{W} = (M - W)(W^\top W)^{1/2} + (WW^\top)^{1/2}(M - W) \qquad \square\]

\textbf{(d)} With \(F = M - W\), the equation from (c.iv) reads:
...
conditions \(W_{l+1}^\top W_{l+1} = W_l W_l^\top\) for all \(l\), and
extends to non-diagonal matrices via the polar decomposition.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Problem 3 --- Rich Regime: Incremental Learning (25
min)}\label{problem-3-rich-regime-incremental-learning-25-min}

\subsubsection{Solution 3}\label{solution-3}

\textbf{(a.i)} Under the alignment assumption,
...
\textbf{(b)} From the logistic solution
...
\textbf{Contrast with the lazy regime} (anticipating Problem 4): in the
...
\textbf{(c)} With uniform initialization \(w_\alpha(0) = w_0\) for all
...
1 has grown significantly. The student is approximately
\(W(t) \approx w_1(t)\,u_1 v_1^\top\), which is \textbf{rank 1}.

\textbf{Intermediate times}: As \(t\) increases, \(w_2\) switches on,
then \(w_3\), etc. The student is approximately
\(W(t) \approx \sum_{\alpha=1}^{k(t)} w_\alpha(t)\,u_\alpha v_\alpha^\top\)
where \(k(t)\) increases stepwise. The \textbf{rank increases
incrementally}.
...
\textbf{Connection to the saddle structure (Problem 1c):} The saddle
...
\textbf{(a)} Starting from the same ODE \(\dot{w} = 2w(s - w)\), with
...
\textbf{(b)} The linearized ODE \(\dot{w} = 2w_0(s - w)\) is first-order
...
sigmoid of Problem 3). The learning timescale is:
...
\textbf{(c)} Restoring the mode index, each mode satisfies
...
Learning time for mode \(\alpha\) & \(t_\alpha \propto 1/s_\alpha\) &
\(t_\alpha \approx \text{const}\) \\
Timescale separation & Strong: \(t_1 \ll t_2 \ll \cdots\) & None \\
Intermediate solutions & Low-rank (incremental) & Full-rank from the
start \\
...
\textbf{(a)} The interpolating ODE is
...
the logistic ODE from Problem 3. \(\square\)
...
--- the frozen-NTK regime of Problem 4 (with \(\tau\) playing the role
of \(w_0\)). \(\square\)

\textbf{(b)} At initialization,
...
\textbf{(c)} In the lazy phase, each mode grows as
...
same mechanism identified by Kumar et al.~in the grokking exercise
session. \(\square\)
...
\textbf{(a)} Setting \(\mathbf{j} = 0\) (thermal equilibrium) with
...
\textbf{(b)} The effective temperature is \(T = \eta\sigma^2/2\).
...
\textbf{(c)} When \(\Sigma(\theta)\) is anisotropic, the noise strength
