...
\section{Learning dynamics of
Deep Linear Networks}\label{implicit-regularization-in-deep-linear-networks}
...
\(\mathcal{L} = \frac{1}{2}\|M - W_2 W_1\|_F^2\), assume that each weight matrix $W_i$ is diagonal and that the inputs are whitened.
...
\textbf{(a)} \textbf{Frozen-NTK solution.} Solve the linearized ODE
above. Show that:
...
\textbf{(b)} \textbf{No timescale separation.} Now restore the mode
