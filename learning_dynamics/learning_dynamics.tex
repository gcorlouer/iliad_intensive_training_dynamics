...
\begin{frame}{How: making mathematical models of learning}
    \begin{itemize}
        \item Tractable toy models: deep linear networks, shallow nonlinear networks, polynomial activations
        ...
        \item Identify useful coarse-grained variables: singular values of weights, learning coefficients, correlation functions
...
\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{figures/emergent_misalignment.png}
\end{frame}
\sectionframe{Setup}
...
\end{aligned}
\]
A neural network is a function $f_\theta$ such that
\[
f_\theta(x) = W_Lh_{L-1}(\theta,x)+b_L.
\]
...
The function is linear in the data and nonlinear in the parameters:
  \[
    \begin{aligned}
...
\small
Learn a function $g$ from observations $y = g(x)$. Initialize neural weights at $\theta_0 \sim \mathcal{N}(0,\sigma^{-\gamma}I)$, with initialization scale $\gamma$.
...
\end{equation*}
Initialize $\theta_0 \sim \mathcal{N}(0,\sigma^{-\gamma}I)$, where $\gamma$ controls the initialization scale. The gradient flow is
\[
...
\begin{frame}{Conserved quantities in DLNs}
  ...
  Along the flow, $G_\ell=\beta_\ell$ is constant. When $\beta_\ell=0$, we have \textbf{balanced weights}.
...
Initialize weights: $\theta \sim \mathcal{N}(0,\sigma^{-\gamma/2}I)$
\\
...
\begin{frame}{Rich regime}
  \small
  For balanced and aligned weights (or $\gamma > 1$), the singular values of the student satisfy
  \[
...
\begin{frame}{Takeaways}
  \begin{block}{Key points}
    \begin{itemize}
      ...
      \item Gradient flow has a simplicity bias toward low-rank approximation
